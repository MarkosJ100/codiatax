...
import { Search, FileDown, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Plus } from 'lucide-react';
...
const History: React.FC = () => {
    ...
    const groupedServices = useMemo(() => {
        ...
        // Object.keys is already sorted correctly from the flat list sorting above, but sorting again to be safe
                ...
                {filteredServices.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        {groupedServices.map((group) => (
                            <div key={group.dateStr} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                <div 
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        {collapsedDates[group.dateStr] ? <ChevronDown size={18} /> : <ChevronUp size={18} />}
                                        <h4 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 'bold', textTransform: 'capitalize' }}>
                                            {format(group.date, "EEEE, d 'de' MMMM", { locale: es })}
                                        </h4>
                                    </div>
                                    <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'var(--text-primary)' }}>
                                        {group.services.length} servicio{group.services.length !== 1 ? 's' : ''} • <span style={{ color: 'var(--success)' }}>{group.totalAmount.toFixed(2)} €</span>
                                    </span>
                                </div>
                                {!collapsedDates[group.dateStr] && group.services.map((service, index) => (
                                    <div key={service.id || index} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem' }}>
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                                                {format(new Date(service.timestamp), 'HH:mm', { locale: es })}
                                            </div>
