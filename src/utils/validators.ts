export const validators = {
    ...
    isValidKilometers: (value: string | number): boolean => {
        const num = typeof value === 'string' ? parseFloat(value) : value;
        return !isNaN(num) && num >= 0 && num < 10000; // Allow 0, Max 10,000 km per day
    },
    ...
    isValidAppPin: (pin: string): boolean => {
        return /^\d{4}$/.test(pin);
    }
};
...
export const getErrorMessage = (field: string, validationType: ValidationKey): string => {
