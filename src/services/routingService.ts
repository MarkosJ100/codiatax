...
export const calculateRoute = async (
    ...
    try {
        ...
        return {
            ...
            duration: 0,
            success: false,
            error: 'Error de conexiï¿½n al servicio de rutas'
        };
    }
};
...
export const geocodeAddress = async (address: string): Promise<GeocodingResult> => {
    try {
        ...
        const query = `${address}, Cï¿½diz, Spain`;
        ...
        if (!data || data.length === 0) {
            return {
                ...
                displayName: '',
                success: false,
                error: 'Direcciï¿½n no encontrada'
            };
        }
        ...
        return {
            ...
            displayName: '',
            success: false,
            error: 'Error de conexiï¿½n al servicio de geocodificaciï¿½n'
        };
    }
};
...
const shortenDisplayName = (fullName: string): string => {
    ...
    const filtered = parts.filter(p =>
        !p.toLowerCase().includes('espaï¿½a') &&
...
export const getLocationSuggestions = async (query: string): Promise<LocationSuggestion[]> => {
    ...
    try {
        // Search in Spain, prioritizing Cï¿½diz area for local results
